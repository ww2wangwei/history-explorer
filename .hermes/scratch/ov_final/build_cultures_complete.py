...
# Apply enrichment and write back to src/data/culture-events.json
import json
data_enriched = []
for orig in data:
    eid = orig['id']
    if eid in ENRICH:
        # Merge: original fields + enrichment fields (preserves id, title, year,
        # category, location, region, importance, description)
...
SRC.write_text(json.dumps(data_enriched, ensure_ascii=False, indent=2), encoding='utf-8')
print(f"Updated: {SRC}")
