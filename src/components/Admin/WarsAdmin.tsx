  ...
  return (
    <div className="p-6 max-w-3xl space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {isDeleted ? (
            <button onClick={() => undelete(war.id)} className="px-3 py-1.5 rounded-lg bg-emerald-700/40 hover:bg-emerald-600/60 border border-emerald-600/50 text-emerald-200 text-xs">↩️ 恢复</button>
          ) : (
            <button onClick={() => {
          ...
          {isEdited && !isNew && !isDeleted && (
            <button onClick={() => { if (confirm(`撤销 "${war.title}" 的所有编辑？`)) deleteOverride(war.id) }} className="px-3 py-1.5 rounded-lg bg-ink-700/60 hover:bg-ink-600 border border-ink-600 text-ink-300 text-xs">↺ 撤销编辑</button>
          )}
        </div>
      </div>
