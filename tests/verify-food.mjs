// verify-food.mjs — 端到端验证 food 子分类 (30 条)
import { chromium } from 'playwright'

const BASE = 'http://localhost:5173/history/'
const EXPECTED_TOTAL = 304
const KEYWORDS = ['筷子', '八大菜系', '孔府宴', '冠', '美食知时节']

const browser = await chromium.launch({ headless: true })
const ctx = await browser.newContext({ viewport: { width: 1440, height: 900 } })
const page = await ctx.newPage()

console.log('=== verify-food (30 条) ===\n')
...
try {
  await page.waitForSelector('text=衣食', { timeout: 5000 })
  console.log('[1] "衣食" chip 已渲染')
} catch {
...
const chipText = await page.evaluate(() => {
  ...
  const btn = btns.find(b => /衣食\s*\(\d+\)/.test(b.textContent || ''))
  if (btn) {
    btn.click()
...
console.log(`[3] 点击的 food chip: "${chipText}"`)
await page.waitForTimeout(1500)

const foodCards = await page.evaluate(() => {
  return document.querySelectorAll('[style*="border-left-width"]').length
})
console.log(`[4] food 子分类卡片数: ${foodCards} (期望 = 30 +/- 浮动)`)
const foodCardsOk = (foodCards >= 30 && foodCards <= 32)
...
console.log(`[5] food 卡片标题 (${titles.length} 条):`)
...
await page.screenshot({ path: '_dbg-food-verify.png', fullPage: true })
...
const checks = [
  ...
  { name: `food 子分类卡片数 (30 +/- 浮动)`, pass: foodCardsOk, actual: foodCards },
  { name: `${KEYWORDS.length} 个食物关键词全部匹配`, pass: keywordsOk, actual: `${found.length}/${KEYWORDS.length}` },
]
let allPass = true
