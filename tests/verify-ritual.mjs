// verify-ritual.mjs — 端到端验证 ritual 子分类 (30 条)
import { chromium } from 'playwright'

const BASE = 'http://localhost:5173/history/'
const EXPECTED_TOTAL = 277
const KEYWORDS = ['成人礼', '婚礼', '科举', '乡射礼', '宗族']

const browser = await chromium.launch({ headless: true })
const ctx = await browser.newContext({ viewport: { width: 1440, height: 900 } })
const page = await ctx.newPage()

console.log('=== verify-ritual (30 条) ===\n')
...
try {
  await page.waitForSelector('text=礼仪制度', { timeout: 5000 })
  console.log('[1] "礼仪制度" chip 已渲染')
} catch {
...
const chipText = await page.evaluate(() => {
  ...
  const btn = btns.find(b => /礼仪制度\s*\(\d+\)/.test(b.textContent || ''))
  if (btn) {
    btn.click()
...
console.log(`[3] 点击的 ritual chip: "${chipText}"`)
await page.waitForTimeout(1500)

const ritCards = await page.evaluate(() => {
  return document.querySelectorAll('[style*="border-left-width"]').length
})
console.log(`[4] ritual 子分类卡片数: ${ritCards} (期望 = 30 +/- 浮动)`)
const ritCardsOk = (ritCards >= 30 && ritCards <= 32)
...
console.log(`[5] ritual 卡片标题 (${titles.length} 条):`)
...
await page.screenshot({ path: '_dbg-ritual-verify.png', fullPage: true })
...
const checks = [
  ...
  { name: `ritual 子分类卡片数 (30 +/- 浮动)`, pass: ritCardsOk, actual: ritCards },
  { name: `${KEYWORDS.length} 个礼仪关键词全部匹配`, pass: keywordsOk, actual: `${found.length}/${KEYWORDS.length}` },
]
let allPass = true
