// verify-tech.mjs — 端到端验证 tech 子分类 (32 条)
import { chromium } from 'playwright'

const BASE = 'http://localhost:5173/history/'
const EXPECTED_TOTAL = 359
const KEYWORDS = ['四大发明', '本草纲目', '圆周率', '黑火药', '天工开物']

const browser = await chromium.launch({ headless: true })
const ctx = await browser.newContext({ viewport: { width: 1440, height: 900 } })
const page = await ctx.newPage()

console.log('=== verify-tech (32 条) ===\n')
...
try {
  await page.waitForSelector('text=科技', { timeout: 5000 })
  console.log('[1] "科技" chip 已渲染')
} catch {
...
const chipText = await page.evaluate(() => {
  ...
  const btn = btns.find(b => /科技\s*\(\d+\)/.test(b.textContent || ''))
  if (btn) {
    btn.click()
...
console.log(`[3] 点击的 tech chip: "${chipText}"`)
await page.waitForTimeout(1500)

const techCards = await page.evaluate(() => {
  return document.querySelectorAll('[style*="border-left-width"]').length
})
console.log(`[4] tech 子分类卡片数: ${techCards} (期望 = 32 +/- 浮动)`)
const techCardsOk = (techCards >= 32 && techCards <= 34)
...
console.log(`[5] tech 卡片标题 (${titles.length} 条):`)
...
await page.screenshot({ path: '_dbg-tech-verify.png', fullPage: true })
...
const checks = [
  ...
  { name: `tech 子分类卡片数 (32 +/- 浮动)`, pass: techCardsOk, actual: techCards },
  { name: `${KEYWORDS.length} 个科技关键词全部匹配`, pass: keywordsOk, actual: `${found.length}/${KEYWORDS.length}` },
]
let allPass = true
